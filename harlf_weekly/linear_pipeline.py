"""
Linear Meta Learning Pipeline - Dedicated pipeline for linear meta agents.
This pipeline is optimized for linear regression-based meta learning at Level 3.
"""
...
class LinearMetaLearningPipeline:
    """Pipeline optimized for linear meta learning approach."""
    ...
    def __init__(
        ...
        self.l3_alpha = l3_alpha
        
        # Initialize agents
        self.tech_agent = TechnicalAgent()
        self.sent_agent = SentimentAgent()
        self.super_agent = SuperAgent()
        self.meta_agent = LinearMetaAgent(n_assets=len(tickers), alpha=l3_alpha)
    ...
    def train_level3(self):
        """Train Level 3 Linear Meta Agent."""
        print("\n" + "="*70)
        print("LEVEL 3: TRAINING LINEAR META AGENT")
        print("="*70)
        
        # Get training data
        train_super_weights, train_macro, train_returns = self._get_meta_training_data('train')
        
        # Train meta agent
        self.meta_agent.fit(
            super_weights=train_super_weights,
            macro_features=train_macro,
            returns=train_returns
        )
        
        print("Linear Meta Agent trained")
    ...
    def _prepare_level3_datasets(self):
        """Prepare Level 3 datasets with L2 predictions."""
        print("Generating Level 3 predictions (NO look-ahead)...")
    ...
    def _get_meta_training_data(self, split: str = 'train'):
        """Get meta training data for Level 3."""
        datasets_split = self.datasets_l3
        n_timesteps = len(datasets_split[self.tickers[0]][split])
        
        # Get super agent weights
        super_weights = []
        for t in range(n_timesteps):
            obs_list = []
            for ticker in sorted(self.tickers):
                asset_data = datasets_split[ticker][split].iloc[t].values
                obs_list.append(asset_data)
            
            # Stack observations - shape should be (n_features, n_assets)
            obs = np.column_stack(obs_list).astype(np.float32)
        ...
        # Get macro features
        macro_features = self.macro_data.loc[datasets_split[self.tickers[0]][split].index].values
        
        # Get returns
        returns_data = []
        for ticker in self.tickers:
            rets = self.returns.xs(ticker, level='ticker').loc[datasets_split[ticker][split].index]
            returns_data.append(rets.values)
        
        returns = np.column_stack(returns_data)
        
        return super_weights, macro_features, returns
    ...
    def _save_level3_models(self, save_dir='models/level3'):
        ...
        os.makedirs(save_dir, exist_ok=True)
        if self.meta_agent:
            with open(os.path.join(save_dir, 'linear_meta_agent.pkl'), 'wb') as f:
                pickle.dump(self.meta_agent, f)
            print(f"✓ Linear meta agent saved to {save_dir}/linear_meta_agent.pkl")
    ...
    def load_level3_models(self, save_dir='models/level3'):
        ...
        import os
        import pickle
        if os.path.exists(os.path.join(save_dir, 'linear_meta_agent.pkl')):
            with open(os.path.join(save_dir, 'linear_meta_agent.pkl'), 'rb') as f:
                self.meta_agent = pickle.load(f)
    ...
    def evaluate(self, split='test'):
        ...
        # Get test data
        super_weights, macro_features, returns = self._get_meta_training_data(split)
        
        # Get meta agent predictions
        meta_predictions = []
        for i in range(len(super_weights)):
            pred = self.meta_agent.predict(super_weights[i], macro_features[i])
            meta_predictions.append(pred)
        
        meta_predictions = np.array(meta_predictions)
        
        # Calculate portfolio returns
        portfolio_returns = (meta_predictions * returns).sum(axis=1)
