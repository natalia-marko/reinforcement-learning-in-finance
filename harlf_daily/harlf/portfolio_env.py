...
from harlf.rewards import EMASharpeReward, MultiObjectiveReward, SimpleReturnReward
...
class PortfolioEnv(gym.Env):
    ...
    def __init__(
        ...
        # Dynamically determine number of features from actual data
        self.n_features = len(self.feature_cols)

        # ✅ FIXED: Observation space now includes current portfolio weights
        # Format: [asset_features (n_features × n_assets), current_weights (n_assets)]
        # This allows agent to optimize transaction costs and implement "stay the course" strategies
        obs_dim = self.n_features * self.n_assets + self.n_assets
    ...
    def _load_data(self):
        ...
        # Extract features (exclude 'ticker' and target columns)
        # Target columns should NOT be part of the observation space
        target_cols = ['forward_log_return', 'forward_return']
        self.feature_cols = [
            col for col in self.data.columns
            if col != 'ticker' and col not in target_cols
        ]

        # Create pivot tables for efficient access
        # Each pivot: rows=dates, cols=tickers, values=feature
        # Include both features AND targets in pivots (targets needed for returns/validation)
        all_cols = self.feature_cols + target_cols
        self.feature_pivots = {}
        for col in all_cols:
            if col in self.data.columns:
                self.feature_pivots[col] = self.data.pivot_table(
    ...
    def _validate_data(self):
        ...
        if 'forward_log_return' not in self.data.columns:
            raise ValueError(
    ...
    def _get_observation(self, step: int) -> np.ndarray:
        ...
        ✅ FIXED: Now includes current portfolio weights for transaction cost optimization
        Returns flattened feature vector: [ticker1_feats, ticker2_feats, ..., current_weights]
        Shape: (n_features × n_assets + n_assets,)
        Excludes target columns (forward_log_return, forward_return)
        """
        ...
        # ✅ FIXED: Append current portfolio weights
        # This allows agent to see current allocation and optimize rebalancing costs
        obs.extend(self.current_weights)
    ...
    def _get_daily_returns(self, step: int) -> np.ndarray:
        ...
        for ticker in self.tickers:
            ...
            # Increased threshold to 0.35 to reduce warnings for extreme but valid events (e.g., earnings crashes)
            if abs(log_return) > 0.35 and not np.isnan(log_return):
    ...
    def step(
        ...
        # ✅ FIXED: Exact portfolio log return calculation (not approximation)
        # Convert log returns to simple returns, compute weighted average, convert back to log
        # This is mathematically correct for all return magnitudes
        asset_simple_returns = np.exp(asset_log_returns) - 1.0
        portfolio_simple_return_gross = np.dot(action, asset_simple_returns)
        portfolio_log_return_gross = np.log(1.0 + portfolio_simple_return_gross)
